...
def _list_runs(storage: "StorageEngine", limit: int) -> None:
    ...
    if not runs:
        console.print(Panel(
            "[dim]No runs found. Instrument your code with [bold cyan]@tz.trace[/] and run it.[/dim]",
...
def _inspect_run(storage: "StorageEngine", run_id: str, show_spans: bool) -> None:
    ...
    if not matched:
        ...
        raise SystemExit(1)
    if len(matched) > 1:
        console.print(f"[yellow]Ambiguous run ID prefix. Matches: {[r.run_id for r in matched]}[/yellow]")
        raise SystemExit(1)
    ...
    info_lines = [
        ...
        f"[bold cyan]Duration:[/bold cyan]  {run.duration_ms:.0f}ms" if run.duration_ms else "[bold cyan]Duration:[/bold cyan]  running…",
