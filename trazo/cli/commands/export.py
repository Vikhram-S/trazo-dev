...
@click.option(
    "--format", "fmt", type=click.Choice(["json", "html"]), default="json", show_default=True
)
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .span-card {{ background: var(--surface); border: 1px solid var(--border);
  ...
  .span-meta {{ font-size: .8rem; color: var(--muted); display: flex;
                gap: 1rem; margin-top: .4rem; }}
  .span-body {{ display: none; margin-top: 1rem; border-top: 1px solid var(--border);
                padding-top: 1rem; }}
  .span-body.open {{ display: block; }}
  pre {{ background: #0d1117; border-radius: 6px; padding: .75rem; font-size: .78rem;
...
<div class="stats">
  ...
  <div class="stat">
    ...
    <div class="stat-value">{run_dict["span_count"]}</div>
  </div>
  <div class="stat">
    <div class="stat-label">Total Tokens</div>
    <div class="stat-value">
      {run_dict["total_tokens_in"] + run_dict["total_tokens_out"]:,}
    </div>
  </div>
  <div class="stat">
    <div class="stat-label">Cost (USD)</div>
    <div class="stat-value">${run_dict["total_cost_usd"]:.6f}</div>
  </div>
</div>
...
spans.forEach((s, i) => {{
  ...
  div.innerHTML = `
    ...
    <div class="span-body" id="body-${{i}}">
      ...
      ${{s.outputs ? `
        <div class="label" style="margin-top:.75rem">Outputs</div>
        <pre>${{JSON.stringify(s.outputs, null, 2).substring(0,2000)}}</pre>
      ` : ''}}
      ${{s.error ? `
        <div class="label" style="margin-top:.75rem;color:var(--red)">Error</div>
        <pre style="color:var(--red)">${{s.error.substring(0,1000)}}</pre>
      ` : ''}}
    </div>
  `;
